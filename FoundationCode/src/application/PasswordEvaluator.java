...
public class PasswordEvaluator {
	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			// valid transitions. Once a check is true it will not repeated to the user.
			if (currentChar >= 'A' && currentChar <= 'Z' && !foundUpperCase) {
				foundUpperCase = true;
			} else if (currentChar >= 'a' && currentChar <= 'z' && !foundLowerCase) {
				foundLowerCase = true;
			} else if (currentChar >= '0' && currentChar <= '9' && !foundNumericDigit) {
				foundNumericDigit = true;
			} else if ("~`!@#$%^&*()_-+{}[]|:,.?/".indexOf(currentChar) >= 0 && !foundSpecialChar) {
				foundSpecialChar = true;
			} else if(!foundOtherChar) { 						
				foundOtherChar = true;
			}
			if (currentCharNdx >= 7 && !foundLongEnough) {
	...
	public static void main(String[] args)
	{
		...
		while(input.hasNextLine())
		{
			...
			if (userInput.length() == 0) {			// If the length of the trimmed line is zero, stop the loop
				System.out.println("\n*** Empty input line detected, the loop stops.");
				input.close();					// Display the reason for terminating the loop.
				System.exit(0);
			}
			
			String errMessage = evaluatePassword(userInput);
	        ...
	        System.out.println("\nPlease enter Password or an empty line to stop.");
		}
	}
